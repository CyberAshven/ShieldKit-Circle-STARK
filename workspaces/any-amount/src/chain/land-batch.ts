...
/**
 * Envelope A is standard-size and relays over electrum. B is ~500 KB and
 * NON-STANDARD, so it needs a direct node RPC (BCHN_RPC_URL or the Start9
 * nsenter path) - electrum will not relay it. That is a transport limit, not a
 * validity one: both verify identically on the VM.
 */
export async function landBatch(args: {
  ...
  const GENESIS_NEED = consensus ? 1_200_000 : 400_000;
  ...
  try {
    ...
    // Consensus mode: B is ~500 KB and non-standard by design.
    ...
    const sourceOutputs = [
      ...
      { lockingBytecode: compileFriQueryLockP2sh32(), valueSatoshis: kSats + successorFeeCoinSats(args.envelope) },
    ...
    const vm = createVirtualMachineBch2026(!consensus).verify({ transaction: tx, sourceOutputs });
