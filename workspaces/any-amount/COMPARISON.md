...
| On-chain verify | BCH 2026 VM: pool lock + 10 kernel inputs walk all 252 FRI openings (~64 KB tx); digest-only / rewritten noteRoot fail | Consensus ring/BP | Consensus SNARK | L2 rollup circuits | Solana Groth16 verifier ELF | Ethereum Groth16 verifier |
| Any-amount | Yes (one set) | Yes | Yes | Yes | Nova-style amounts | Classic fixed tickets |
| Amount hiding | Pedersen in the note leaf; public path is **net** UTXO value (`STATE_BASE + reserve`); PAA1 still carries reserve+sequence | Yes (BP) | Yes | Yes | Yes | No (ticket size public) |
...
- Hidden UTXO **value** on BCH (sats on the pool output stay public; only note amounts are committed).
