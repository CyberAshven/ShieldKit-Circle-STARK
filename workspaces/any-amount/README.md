...
| On-chain Circle FRI prefix | Pool lock + **10** Merkle kernels + bind-T + grind + algebraicC + fold + R-slots. Envelopes: **A** occupancy standard (FRI10 QM31, 18-input; may be incomplete) in [`research-lanes/sha-in-occupancy-c`](../../research-lanes/sha-in-occupancy-c/); **B** completeness at consensus size; **C** B chunked. Product CLI `--envelope a` here is still FRI9 4-slot until occupancy is wired. Historical Chipnet txids stay in [`MILESTONE.md`](MILESTONE.md). See [`ENVELOPES.md`](ENVELOPES.md). Unlocking **and** redeem ≤ **10 KB**. Dummy 99 KB cargo is not the verifier (`packTo` default 0). |
