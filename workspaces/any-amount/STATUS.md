...
- BCH 2026 VM (libauth CashAssembly, not OP_RETURN): pool five-point + spent-note preimage; PAA1 NFT is the public cell. Inputs 1..10 Merkle-walk packed `layerRoots`. Input 11 binds Newton `T` to AIR cells (0,1,2,3,18,23,24) and those cells to the statement (reserves, action, amount, sequences, digest). Input 12 runs slot-0 `C=Q·Z` (`N` from `T`, `Z≠0`). Cooking `T` or `nTable=Q'·Z` fails. Dummy 8-leaf kernels fail. Density (`unlocking*800`) does not fit all 36 FS slots in one input; remaining slots are still JS `verifyFri`.
