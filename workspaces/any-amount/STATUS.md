...
R-slots (`(q−R)·Z = N` from T). Envelope **B** is one consensus tx ≤ **1 MB**
...
pay hop is B, including the note-auth kernel (Merkle + nullifier + amount/auth).
Tape hops are extra real fold/R-slot slices (18×2 queries, 100 KB
each), not dummy cargo and not a cross-tx accumulator. Skip-tape still hits the
B pay hop. `packTo` defaults to **0**. 99 KB packing is not the verifier.
...
| Statistical ZK openings? | **On-chain R, not a Lean HVZK theorem.** Prove/verify and the slot lock use \(R_{\mathrm{on}}(i)+Z(i)R_{\mathrm{off}}(i)\) (SHA-256 coeffs; Z FS-bound; off-domain \(Z_H\cdot R\) as in ePrint 2024/1037). Slot check is `(qTable−R)·Z` against AIR N from T, not masked nTable. Not a Lean HVZK theorem. |
...
| Opened FRI view | **Can raise, not a theorem.** Slot kernels evaluate \(R_{\mathrm{on}}(i)+Z(i)R_{\mathrm{off}}(i)\) (SHA-256 coeffs, deg 7 / 35, ePrint 2024/1037-style off-domain, Z FS-bound) and check `(qTable−R)·Z` against AIR N from T. Opened diffs are not plaintext \(Q\) diffs. Not a Lean HVZK theorem. |
...
- N-payout lock is shipped (sum + HASH256 of every lock+value; withdrawalCount delta = payout count). Envelope B walks one opened note + nullifier on-chain; batch-exit extra notes and envelope A still stay in `verifyFri`. Encoded `plugin.verify` sees one auth (OTP-masked amounts).
...
- More than one FS query folded per redeem (density). Viewing-key delivery / wallet UX. Lean ePrint 2024/1037 HVZK theorem. Slot kernels **do** evaluate \(R\). algebraicC-as-FRI interpolant would bump `FRI_VERSION` (frozen at 8).
