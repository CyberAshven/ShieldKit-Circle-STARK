...
| Statistical ZK openings? | **Shipped mask, not a Lean HVZK theorem.** Prove/verify use \(R_{\mathrm{on}}(i)+Z(i)R_{\mathrm{off}}(i)\) (SHA-256 coeffs; off-domain \(Z_H\cdot R\) as in ePrint 2024/1037). Opened diffs are not plaintext \(Q\) diffs. The 10 KB lock does **not** evaluate \(R\) (masked Q·Z=nTable). Not a Lean HVZK theorem. |
...
- N-payout lock is shipped (sum + HASH256 of every lock+value; withdrawalCount delta = payout count). Notes/N nullifiers still stay in `verifyFri` — the 10 KB lock does not walk the note tree. Encoded `plugin.verify` sees one auth (OTP-masked amounts).
...
- More than one FS query folded per redeem (density). Viewing-key delivery / wallet UX. Lean ePrint 2024/1037 HVZK theorem. The 10 KB lock does not evaluate \(R\) (CashVM still pays `OP_SHA256` Merkle/FS).
