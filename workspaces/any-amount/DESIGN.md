...
Opt-in **batch exit** (`pool withdraw --batch-exit`, knob `--batch-window` default **180 s**) is a **shared round**: the first waiter opens the clock; anyone else who opts in before close is in the same flush. At close, batch whoever is already in. A late arriver after close opens the next round. Per-user min/max random waits were removed because they miss the group. It is not CashFusion (no `OP_RETURN FUSE`, no Pedersen/blind Schnorr). The shipped lock still binds one HASH256 payout at output 1; summing N payouts in one successor is a later lock. Fast single withdraw stays the default.
