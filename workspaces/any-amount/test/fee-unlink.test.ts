...
describe("successor fee unlinked from withdraw", () => {
  it("compiled successor outputs are STATE_BASE + funder change; fee ≠ withdraw", () => {
    const note: Note = {
      amountSats: 20_000n,
      rho: rnd32(),
      ownerSecret: rnd32(),
    ...
    const w = applyWithdraw(d.machine, note, d.index, rnd32(), WITHDRAW_SATS);
    ...
    assert.equal(values.length, 2, "pool + funder change only");
    assert.ok(values.includes(STATE_BASE_SATS), "pool output stays STATE_BASE");
    assert.equal(
      values.includes(WITHDRAW_SATS),
      false,
      "withdraw sats must not appear as an output value",
    );
    const changeOut = values.find((v) => v !== STATE_BASE_SATS);
    assert.ok(changeOut !== undefined, "funder change output");
