...
import { evaluateNoteAuthKernel, evaluatePoolSuccessorVm } from "../src/chain/vm-verifier.ts";
...
import { FRI_PAIR_BYTES_L0, FRI_PAIR_BYTES_QM } from "../src/chain/fri-kernel.ts";
import { defaultInternalHash } from "../src/backends/circle/internal-hash.ts";
import { SHA_LDE_N_LEAVES, SHA_LDE_VALUE_BYTES, openShaLde } from "../src/chain/sha-lde.ts";
...
describe("shielded unlocking envelope-B successor", () => {
  ...
  it(
    ...
    () => {
      ...
      const junkLeaves = Array.from({ length: SHA_LDE_N_LEAVES }, () => {
        const v = new Uint8Array(SHA_LDE_VALUE_BYTES);
        v.set(mixedOpens.amountCommit.subarray(0, 4), 0);
        v.set(mixedOpens.leaf.subarray(0, 4), 4);
        v.set(mixedOpens.nf.subarray(0, 4), 8);
        crypto.getRandomValues(v.subarray(12));
        return v;
      });
      const h = defaultInternalHash();
      const junkRoot = new MerkleTree(junkLeaves, h).root;
      const recooked = proveFromTLde(statement2, qMixedLeaves, auth2, {
        hashRoot: pin,
        hashLeaves: mixedLeaves,
        hashBitRoot: junkRoot,
      });
      const junkLde = openShaLde(junkLeaves, recooked.queries.map((q) => q.index), h);
      const junkBits = proveFromTLde(statement2, qMixedLeaves, auth2, {
        hashRoot: pin,
        hashLeaves: mixedLeaves,
        hashBitRoot: junkRoot,
        hashBitLde: junkLde,
      ...
      const occQ = algebraicCQuotientLde(statement2, circleDomain(TRACE_LEN), circleDomain(FRI_N)).qLde;
      const recookedOcc = proveFromTLde(statement2, occQ, auth2, {
        hashRoot: pin,
        hashLeaves: mixedLeaves,
        hashBitRoot: junkRoot,
      });
      const junkLdeOcc = openShaLde(
        junkLeaves,
        recookedOcc.queries.map((q) => q.index),
        h,
      );
      const occJunk = proveFromTLde(statement2, occQ, auth2, {
        hashRoot: pin,
        hashLeaves: mixedLeaves,
        hashBitRoot: junkRoot,
        hashBitLde: junkLdeOcc,
      });
