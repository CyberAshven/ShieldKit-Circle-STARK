...
import { add, encodeLe, inv, M31, mul, sub, type M31El } from "./m31.ts";
import {
  decodeQm31,
  encodeQm31,
  hashBytesToQm31,
...
export type FriProof = {
  ...
  queries: FriQuery[];
  auth: FriAuth;
  /** SHA256(leaf‖nf‖amountCommit‖createdLeaf). Miner-bound in packed AIR. */
  hashRoot?: Uint8Array;
...
function queryGrindSeed(
  ...
  even: Uint8Array,
  odd: Uint8Array,
  hashRoot: Uint8Array = new Uint8Array(32),
  hashBitRoot: Uint8Array = new Uint8Array(32),
): Uint8Array {
  return hash.digest(concatBytes(digest, traceRoot, ...layerRoots, even, odd, hashRoot, hashBitRoot));
}
...
/**
 * Attach a foreign SHA-LDE, re-grind, and re-open occupancy queries at the
 * new FS indices (same path as proveFromTLde). Occupancy FRI stays valid;
 * hash-bit prefixes follow the new LDE.
 */
export function swapShaLdeAndRegrind(
  proof: FriProof,
  ldeLeaves: Uint8Array[],
  statement: PoolStatement,
  ...
  const bitRoot = new MerkleTree(ldeLeaves, hash).root;
  ...
  const qIdx = occupancy.queries.map((q) => q.index);
  return {
    ...occupancy,
    hashBitRoot: bitRoot,
    hashBitLde: openShaLde(ldeLeaves, qIdx, hash),
  };
}

/** Foreign amountCommit bit-AIR, re-grind, re-open occupancy at the new FS indices. */
export function swapShaBitAndRegrind(
  proof: FriProof,
  witness: Parameters<typeof shaBitLdeLeaves>[0],
  statement: PoolStatement,
  hash: InternalHash = defaultInternalHash(),
): FriProof {
  const small = circleDomain(TRACE_LEN);
...
export function verifyFri(
  ...
  const hashRoot = proof.hashRoot && proof.hashRoot.length === 32 ? proof.hashRoot : new Uint8Array(32);
  const hashBitRoot =
    proof.hashBitRoot && proof.hashBitRoot.length === 32 ? proof.hashBitRoot : new Uint8Array(32);
  const expectBind = noteAuthBindFromStatement(statement, proof.auth);
  if (hashRoot.some((b) => b !== 0) && !eq32(hashRoot, expectBind)) {
    return { ok: false, reason: "hashRoot" };
  }
  const grindSeed = queryGrindSeed(
    hash,
    digest,
  ...
  const opens = noteAuthOpensFromStatement(statement, proof.auth);
  const want = shaPublicPrefix(opens);
  if (proof.shaBit) {
    const sb = proof.shaBit;
    if (hashBitRoot.some((b) => b !== 0) && !eq32(sb.root, hashBitRoot)) {
      return { ok: false, reason: "sha-bit root" };
    }
    for (let q = 0; q < SHA_BIT_QUERIES; q += 1) {
      const felts = reconstructFeltsFromShards(sb.shards, q);
      const leaf = concatBytes(want, felts);
      const compact = sb.compact.subarray(q * SHA_BIT_COMPACT, (q + 1) * SHA_BIT_COMPACT);
      if (!walkShaBitLeaf(leaf, compact, sb.table, sb.root, hash)) {
        return { ok: false, reason: "sha-bit walk" };
      }
    }
  } else if (proof.hashBitLde) {
    const lde = proof.hashBitLde;
    if (hashBitRoot.some((b) => b !== 0) && !eq32(lde.root, hashBitRoot)) {
      return { ok: false, reason: "hash-bit root" };
    }
    for (const o of lde.openings) {
      if (!walkShaOpening(o.value, o.compact, lde.table, lde.root, hash)) {
        return { ok: false, reason: "hash-bit walk" };
      }
      if (o.value.length < want.length || want.some((b, i) => o.value[i] !== b)) {
        return { ok: false, reason: "hash-bit prefix" };
      }
    }
  }
...
export function encodeFriProof(p: FriProof): Uint8Array {
  ...
  if (p.shaBit) {
    parts.push(writeU16BE(p.shaBit.compact.length), p.shaBit.compact);
    parts.push(writeU16BE(p.shaBit.table.length), p.shaBit.table);
    for (const s of p.shaBit.shards) parts.push(s);
  } else if (p.hashBitLde) {
    const blob = encodeShaLdeBlob(p.hashBitLde);
    parts.push(writeU16BE(blob.length), blob);
  }
...
export function decodeFriProof(bytes: Uint8Array): FriProof {
  ...
  let hashRoot: Uint8Array | undefined;
  let hashBitRoot: Uint8Array | undefined;
  let hashBitLde: ShaLdeProof | undefined;
  let shaBit: ShaBitProof | undefined;
  if (o + 64 <= bytes.length) {
    hashRoot = bytes.slice(o, o + 32);
    o += 32;
    hashBitRoot = bytes.slice(o, o + 32);
    o += 32;
    if (o + 2 <= bytes.length) {
      const n = readU16BE(bytes, o);
      o += 2;
      if (n === SHA_BIT_QUERIES * SHA_BIT_COMPACT && o + n + 2 + SHA_BIT_FOLDS * SHA_BIT_SHARD_BYTES <= bytes.length) {
        const compact = bytes.slice(o, o + n);
        o += n;
        const nT = readU16BE(bytes, o);
        o += 2;
        const table = bytes.slice(o, o + nT);
        o += nT;
        const shards: Uint8Array[] = [];
        for (let f = 0; f < SHA_BIT_FOLDS; f += 1) {
          shards.push(bytes.slice(o, o + SHA_BIT_SHARD_BYTES));
          o += SHA_BIT_SHARD_BYTES;
        }
        shaBit = { root: hashBitRoot ?? new Uint8Array(32), table, compact, shards, leaves: [] };
      } else if (n > 0 && o + n <= bytes.length) {
        const d = decodeShaLdeBlob(bytes.slice(o, o + n));
        o += n;
        hashBitLde = {
          root: hashBitRoot,
          table: d.table,
          openings: d.leaves.map((value, i) => ({
            index: 0,
            value,
            compact: d.compact.subarray(i * SHA_LDE_COMPACT, (i + 1) * SHA_LDE_COMPACT),
          })),
          leaves: d.leaves,
        };
      }
