 ...
 * Lives in the note-auth unlocking (its own density), not fold leftover.
 * Leaf = public A/L/N prefix (12 B) || masked w-mix (16 × 4 B M31).
 * deg(Z·R) keeps 36 samples from interpolating TRACE.
 */
...
/** 16 consecutive mix felts / leaf → 64-leaf tree, depth 6. Depth 7 overflowed density; bundle 32 made value-SPLITS worse. */
export const SHA_LDE_BUNDLE = 16;
export const SHA_LDE_VALUE_BYTES = SHA_LDE_PREFIX + SHA_LDE_BUNDLE * 4;
/** 64-leaf tree: prefix‖16 consecutive masked w-mix LDE felts. Path depth 6. */
export const SHA_LDE_PATH_DEPTH = 6;
export const SHA_LDE_COMPACT = SHA_LDE_PATH_DEPTH * COMPACT_PATH_STRIDE;
/** deg(Z·R)≈32+600 > 36×16 samples, so TRACE w does not interpolate. */
export const SHA_LDE_OFF_DEGREE = 600;
...
/** Honest cargo, no leftover-fill pad. nTable fits one byte (64-leaf tree). */
export function encodeShaLdeBlob(proof: ShaLdeProof): Uint8Array {
  const nTable = proof.table.length / 32;
  if (nTable > 255) throw new Error(`sha-lde nTable ${nTable}`);
  const values = concatBytes(...proof.openings.map((o) => o.value));
  const compact = concatBytes(...proof.openings.map((o) => o.compact));
  return concatBytes(Uint8Array.of(nTable), values, compact, proof.table);
}

export function encodeShaLdeShards(proof: ShaLdeProof): Uint8Array[] {
  const head = encodeShaLdeBlob(proof);
  if (head.length > SHA_LDE_SLOT) {
    throw new Error(`sha-lde cargo ${head.length} > ${SHA_LDE_SLOT}`);
  }
...
export function decodeShaLdeBlob(blob: Uint8Array): {
  nTable: number;
  values: Uint8Array;
  compact: Uint8Array;
  table: Uint8Array;
