...
import { noteAuthBindFromStatement, noteAuthOpensFromStatement } from "./note-auth-bind.ts";
...
/** SHA256(leaf‖nf‖amountCommit‖createdLeaf). Unused net-length slot in occupancy. */
export const AIR_OFF_NET = 876;
/** Merkle root of occupancy-query SHA-LDE leaves. */
...
export function fiatShamirQueryIndices(
  ...
  const grindSeed = hash.digest(
    concatBytes(digest, proof.traceRoot, ...proof.layerRoots, even, odd, hashRoot, hashBitRoot),
  );
...
export function encodeAirPacked(
  ...
  const opens = noteAuthOpensFromStatement(statement, p.auth);
  const authBind =
    p.hashRoot && p.hashRoot.length === 32 && p.hashRoot.some((b) => b !== 0)
      ? p.hashRoot
      : noteAuthBindFromStatement(statement, p.auth);
  ...
  const hashLeaves =
    p.hashLeaves && p.hashLeaves.length >= 96
      ? p.hashLeaves.subarray(0, 96)
      : concatBytes(opens.amountCommit, opens.leaf, opens.nf);
  packed.set(hashLeaves, AIR_OFF_CELLS + HASH_CELL_COMMIT * 4);
...
/** Packed on top (DUP first). 8×4-byte cells starting at `startCell` → 32-byte blob. */
