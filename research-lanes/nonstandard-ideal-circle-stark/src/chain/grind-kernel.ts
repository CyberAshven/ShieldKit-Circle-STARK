...
export function grindKernelUnlocking(packed?: Uint8Array, shaBitPaths?: Uint8Array): Uint8Array {
  ...
  const pushDigest = Uint8Array.of(digest.length, ...digest);
  if (shaBitPaths && shaBitPaths.length >= 32) {
    const pushPaths =
      shaBitPaths.length <= 75
        ? Uint8Array.of(shaBitPaths.length, ...shaBitPaths)
        : shaBitPaths.length <= 255
          ? Uint8Array.of(0x4c, shaBitPaths.length, ...shaBitPaths)
          : Uint8Array.of(0x4d, shaBitPaths.length & 0xff, (shaBitPaths.length >> 8) & 0xff, ...shaBitPaths);
    const out = new Uint8Array(pushPaths.length + pushDigest.length + redeem.length);
    out.set(pushPaths, 0);
    out.set(pushDigest, pushPaths.length);
    out.set(redeem, pushPaths.length + pushDigest.length);
    return out;
  }
  const out = new Uint8Array(pushDigest.length + redeem.length);
  out.set(pushDigest, 0);
  out.set(redeem, pushDigest.length);
  return out;
}
