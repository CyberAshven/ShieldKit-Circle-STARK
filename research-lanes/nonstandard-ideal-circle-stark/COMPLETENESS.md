...
|---|---|---|---|
| 1 | `FRI_VERSION` **10** | 10 | New family + new vk. No silent bump. |
...
| 4 | **On-chain \(R\)** | fused into those fold inputs: `(q−R)·Z = C(z)`, honest C = 0 | Same check, cheaper ASM |
...
| 8 | **Note membership + nullifier + amount/auth on chain as SHA-256 AIR** | occupancy kernel still pushes amount/rho/owner | Extra-input 576-column AIR; step-kernel equivalent for N notes; **not** leftover SHA-LDE; **not** “leave it in `verifyFri`” |
...
| 13 | **No preimage in unlocking** | occupancy still pushes rho/owner/amount8 | Required; AIR extra inputs are the next construction |
