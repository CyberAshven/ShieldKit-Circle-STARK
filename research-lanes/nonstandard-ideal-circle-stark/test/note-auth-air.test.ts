...
describe("note-auth SHA-256 AIR", () => {
  ...
  it("36-query bit-AIR extra-input meter (wall, not a pad)", () => {
    ...
    const occupancyPlusAir = 99397 + m.extraTxBytes;
    assert.ok(occupancyPlusAir > 100000, `sibling 100 KB miss ${occupancyPlusAir}`);
    assert.ok(occupancyPlusAir <= 1_000_000, `consensus 1 MB fit ${occupancyPlusAir}`);
  });
});
