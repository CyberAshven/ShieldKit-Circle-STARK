...
RECEIPT_SCHEMA = (
    "shieldkit-labs/external-launcher/cpsb-exact27-diagnostic/receipt/v2"
)
RECEIPT_LAUNCHER_ID = "cpsb-exact27-diagnostic-v2"
...
VALIDATOR_STDOUT_MAX_BYTES = 65_536
VALIDATOR_STDERR_MAX_BYTES = 65_536
VALIDATOR_PIPE_BYTES = 4_096
VALIDATOR_KILL_WAIT_SECONDS = 5.0
MAX_RECEIPT_BYTES = 400_000
VALIDATOR_ARGV = [
    "/runtime/node",
    "--no-addons",
...
def base_receipt(
    *,
    ...
    return {
        ...
        "launcherId": RECEIPT_LAUNCHER_ID,
        "manifestLauncherId": LAUNCHER_ID,
        "action": action,
        "authorization": "NONE",
        ...
        "validatorExecutionPolicy": {
            ...
            "captureReadBytes": VALIDATOR_PIPE_BYTES,
            "overflowRule": "BYTE_65537_KILLS_AND_REAPS_CHILD",
            "timeoutRule": "TIMEOUT_KILLS_AND_REAPS_CHILD",
        },
        "retention": (
...
def emit_receipt(receipt: dict[str, object]) -> None:
    ...
    if len(encoded) > MAX_RECEIPT_BYTES:
        reject("RECEIPT_BYTES", str(len(encoded)))
...
def output_record(
    ...
    *,
    ...
    """Encode bounded evidence without interpreting child output."""
    captured = bytes(output)
    if complete:
        return {
            "complete": True,
            "bytes": len(captured),
            "bytesHex": captured.hex(),
            "rawSha256": raw_sha256(captured),
            "limitBytes": limit,
        }
    if reason is None or observed_at_least is None:
        reject("DIAGNOSTIC_INCOMPLETE_RECORD")
    if observed_at_least < len(captured):
        reject("DIAGNOSTIC_LOWER_BOUND")
    return {
        "complete": False,
        "reason": reason,
        "capturedPrefixBytes": len(captured),
        "observedAtLeastBytes": observed_at_least,
        "prefixHex": captured.hex(),
        "observedPrefixRawSha256": raw_sha256(captured),
        "limitBytes": limit,
    }
...
def diagnostic_completion(
    *,
    ...
    collector_cause: str,
    limited_stream: str | None,
    eof_streams: set[str],
    stdout: bytearray,
    stderr: bytearray,
) -> dict[str, object]:
    """Keep collector termination distinct from the child exit or signal."""
    returncode = process.returncode
    if not isinstance(returncode, int):
        reject("VALIDATOR_UNREAPED")
    complete = collector_cause == "complete"
    if complete and limited_stream is not None:
        reject("DIAGNOSTIC_COMPLETE_LIMITED")
    if complete and eof_streams != {"stdout", "stderr"}:
        reject("DIAGNOSTIC_INCOMPLETE_EOF")
    if collector_cause == "stream-limit":
        if limited_stream not in {"stdout", "stderr"}:
            reject("DIAGNOSTIC_LIMITED_STREAM")
        stdout_reason = (
            "stream-limit" if limited_stream == "stdout" else "peer-stream-limit"
        )
        stderr_reason = (
            "stream-limit" if limited_stream == "stderr" else "peer-stream-limit"
        )
        stdout_lower_bound = (
            VALIDATOR_STDOUT_MAX_BYTES + 1
            if limited_stream == "stdout"
            else len(stdout)
        )
        stderr_lower_bound = (
            VALIDATOR_STDERR_MAX_BYTES + 1
            if limited_stream == "stderr"
            else len(stderr)
        )
    elif collector_cause == "timeout":
        stdout_reason = "timeout"
        stderr_reason = "timeout"
        stdout_lower_bound = len(stdout)
        stderr_lower_bound = len(stderr)
    elif complete:
        stdout_reason = None
        stderr_reason = None
        stdout_lower_bound = None
        stderr_lower_bound = None
    else:
        reject("DIAGNOSTIC_COLLECTOR_CAUSE")
    return {
        "captureComplete": complete,
        "collectorTermination": {
            "cause": collector_cause,
            "limitedStream": limited_stream,
        },
        "childTermination": {
        ...
        "stdout": output_record(
            stdout,
            complete=complete,
            limit=VALIDATOR_STDOUT_MAX_BYTES,
            reason=stdout_reason,
            observed_at_least=stdout_lower_bound,
        ),
        "stderr": output_record(
            stderr,
            complete=complete,
            limit=VALIDATOR_STDERR_MAX_BYTES,
            reason=stderr_reason,
            observed_at_least=stderr_lower_bound,
        ),
    }
...
def run_validator_diagnostic(
    ...
    """Run exactly one fixed child and return only bounded diagnostic evidence.

    Spawn, pipe, selector, and reaping failures are outer bootstrap failures.
    Byte 65,537 on either stream and deadline expiry immediately kill and reap
    the child; neither path compares, parses, accepts, or retries its output.
    """
    try:
    ...
    try:
        ...
        while selector.get_map() and collector_cause is None:
            remaining = deadline - time.monotonic()
            if remaining <= 0:
                kill_and_reap_validator(process)
                collector_cause = "timeout"
                break
            ...
            for key, _ in events:
                ...
                if not chunk:
                    ...
                    key.fileobj.close()
                    eof_streams.add(name.lower())
                    continue
                if len(output) + len(chunk) > limit:
                    output.extend(chunk[: limit - len(output)])
                    kill_and_reap_validator(process)
                    collector_cause = "stream-limit"
                    limited_stream = name.lower()
                    break
                output.extend(chunk)

        if collector_cause is None:
            remaining = deadline - time.monotonic()
            if remaining <= 0:
                kill_and_reap_validator(process)
                collector_cause = "timeout"
                ...
                try:
                    ...
                    # A child reaped at or after the wall deadline is timeout
                    # evidence, never a complete diagnostic capture.
                    if time.monotonic() >= deadline:
                        collector_cause = "timeout"
        if collector_cause is None:
            collector_cause = "complete"
        return diagnostic_completion(
            process=process,
            collector_cause=collector_cause,
            limited_stream=limited_stream,
            eof_streams=eof_streams,
            stdout=stdout,
            stderr=stderr,
        )
    finally:
...
def run_authenticated(
    *,
    ...
    try:
        ...
        diagnostic = run_validator_diagnostic(args, pass_fds)
        receipt["classification"] = (
            "AUTHENTICATED_UNSEALED_STATIC_DIAGNOSTIC_ONLY_NONACCEPTING"
        )
        receipt["validator"] = {
            "executed": True,
            "argv": VALIDATOR_ARGV,
